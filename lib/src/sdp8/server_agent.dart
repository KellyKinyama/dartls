...
ServerAgent newServerAgent(
    List<String> candidateIPs, int udpPort, String conferenceName) {
  final ufrag = generateICEUfrag();
  final pwd = generateICEPwd();
  ...
  print(
      'A new server ICE Agent was created (for a new conference) with Ufrag: <u>$ufrag</u>, Pwd: <u>$pwd</u>, FingerprintHash: <u>$fingerprintHash</u>');
  return agent;
}

SignalingMediaComponent ensureSignalingMediaComponent(
    ServerAgent agent, String iceUfrag, String icePwd, String fingerprintHash) {
...
String generateICEPwd() {
  ...
  return uuid
      .v4()
      .replaceAll('-', '')
      .substring(0, 24); // Typically 24 characters
}
