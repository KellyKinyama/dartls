...
Attribute createAttrXorMappedAddress(Uint8List transactionID, InternetAddress addr ,int addrPort)  {
  ...
  xorMask.setRange(4, 16, transactionID);
	...
	final portModifier = (((xorMask[0]) << 8) & 0x0000FF00) | ((xorMask[1]) & 0x000000FF);
...
MappedAddress DecodeAttrXorMappedAddress(Attribute attr , Uint8List transactionID ) {
	final xorMask = Uint8List( 16);
  ByteData writer = ByteData.sublistView(xorMask);
	writer.setUint32(0, magicCookie)
	// copy(xorMask[4:], transactionID)
  xorMask.setRange(4, 16, transactionID);
	...
	return MappedAddress(family,InternetAddress.fromRawAddress(xorIP,type: InternetAddressType.IPv4),port^x);
